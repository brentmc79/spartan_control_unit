...
void drawNoise(int intensity, TFT_eSPI* displayPtr) {
  ...
  for (int i = 0; i < intensity; i++) {
    // Draw random pixels
    display.drawPixel(random(0, screenWidth), random(0, screenHeight), TFT_WHITE);
    ...
    if (random(0, 10) > 8) {
      ...
      if (random(0, 2) == 0) { // Horizontal line
        display.drawFastHLine(x, y, random(5, 20), TFT_WHITE);
      } else { // Vertical line
        display.drawFastVLine(x, y, random(2, 10), TFT_WHITE);
      }
    }
  }
}
...
void loadingAnimation2(TFT_eSPI* displayPtr){
  ...
  display.drawRect(0, 60, screenWidth, 2, TFT_WHITE);
  ...
  display.drawRect(progressBarX - 1, progressBarY - 1, progressBarWidth + 2, progressBarHeight + 2, TFT_WHITE);
  
  for (int i = 0; i <= progressBarWidth; i += 6) {
    display.fillRect(progressBarX, progressBarY, i, progressBarHeight, TFT_WHITE);
    //display.display();
...
void loadingAnimation3(TFT_eSPI* displayPtr){
  ...
  for (int i = 0; i < 9; i++) {
    ...
    display.drawPixel(random(0, screenWidth), random(0, screenHeight), TFT_WHITE); // Random pixel noise
    display.drawPixel(random(0, screenWidth), random(0, screenHeight), TFT_WHITE); // Random pixel noise
    delay(30 + random(0, 50));
  }
  ...
  display.drawRect(0, 50, screenWidth, 2, TFT_WHITE); // Horizontal line separator
  ...
  for (int i = 0; i <= progressBarWidth; i += 2) { // Increment by 2 for faster fill and visual effect
    ...
    if (i % 10 == 0) { // Every few steps, redraw some lines
      display.fillRect(0, progressBarY + progressBarHeight + 5, screenWidth, screenHeight - (progressBarY + progressBarHeight + 5), TFT_BLACK); // Clear bottom area
      for (int j = 0; j < 5; j++) {
        display.drawFastHLine(random(0, screenWidth / 2), progressBarY + progressBarHeight + 5 + (j * 3), random(10, 50), TFT_WHITE);
        display.drawFastHLine(random(screenWidth / 2, screenWidth), progressBarY + progressBarHeight + 5 + (j * 3), random(10, 50), TFT_WHITE);
      }
    }
